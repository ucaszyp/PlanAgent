...
class PDMGenerator:
    ...
    def generate_trajectory(
        ...
        for time_idx in range(
            ...
            # self._proposal_sampling.num_poses + 2,
            1,
        ):
    ...
    def _update_leading_agents(
        ...
        if not update_leading_agent:
            ...
            for proposal_idx in lateral_batch_idcs:
                ...
                # for agent, progress in object_progress_dict.items():
                #     dx = (
                #         self._observation[time_idx][agent].centroid.x
                #         - self._observation[time_idx - 1][agent].centroid.x
                #     )
                #     dy = (
                #         self._observation[time_idx][agent].centroid.y
                #         - self._observation[time_idx - 1][agent].centroid.y
                #     )
                #     agent_heading = math.atan2(dy, dx)
                #     heading_diff = abs(
                #         agent_heading
                #         - self._state_array[proposal_idx, time_idx, StateIndex._HEADING]
                #     )
                #     stillness = True if dx == 0 and dy == 0 else False
                #     if progress > current_ego_progress and (
                #         heading_diff < math.pi * (3 / 4) or stillness
                #     ):
                #         agents_ahead[agent] = progress
