...
from tuplan_garage.planning.simulation.planner.pdm_planner.GPT_utils_turn import (
...
from tuplan_garage.planning.simulation.planner.pdm_planner.bev_vis.gpt_vis import get_map, plot_scenario
...
from tuplan_garage.planning.simulation.planner.pdm_planner.scenario_analysis import (
...
class AbstractPDMClosedPlanner(AbstractPDMPlanner):
    ...
    def __init__(
        ...
        self.last_scenario_type = "regular"
    ...
    def _get_adjacent_lanes(self, ego_state: EgoState, offset=4.0):
        ...
        current_outcoming_edges = current_lane.outgoing_edges
        left_outcoming_edges = left_lane.outgoing_edges
        right_outcoming_edges = right_lane.outgoing_edges
        left_route_availabe = False
        right_route_availabe = False
        for left_outcoming_edge in left_outcoming_edges:
            for current_outcoming_edge in current_outcoming_edges:
                if current_outcoming_edge.is_same_roadblock(left_outcoming_edge):
        ...
        for right_outcoming_edge in right_outcoming_edges:
            for current_outcoming_edge in current_outcoming_edges:
                if current_outcoming_edge.is_same_roadblock(right_outcoming_edge):
    ...
    def _get_discrete_centerline_change(
        ...
        if mode == "regular":
            ...
            roadblocks = list(self._route_roadblock_dict.values())
            roadblock_ids = list(self._route_roadblock_dict.keys())
            print("len of roadblocks", len(roadblocks))
    ...
    def _get_gpt_choice(
        self, ego_state, adjacent_lanes, scenario_type, current_input, initialization
    ):
        user_message = ""
        # 得到周围的车辆信息，并将坐标转化成相对坐标
        ...
        if len(dynamic_object_coords.shape) == 3:
            ...
            for dynamic_object_coord in dynamic_object_coords:
        ...
        # lane_position_indicator指示离当前node最近的几个node
        ...
        path="" # path to save the BEV picture
        bev_pic = f"{path}/{randint}"
        plot_scenario(
            bev_data_cache,
            bev_pic,
        )

        user_message += get_scenario_type(scenario_type)
        ...
        user_message += "Max acceleration options: [1.0,1.5,2.0]\n"
        ...
        # print(self.gpt_choice)

        if self.gpt_choice[0] == "left" and adjacent_lanes["left"] is not None:
            chosen_lane = adjacent_lanes["left"]
        elif self.gpt_choice[0] == "right" and adjacent_lanes["right"] is not None:
            chosen_lane = adjacent_lanes["right"]
        else:
            chosen_lane = adjacent_lanes["current_lane"]
        return chosen_lane, self.gpt_choice, abstastic_flag

    def _get_simulated_proposals(
        self, ego_state, chosen_lane, scenario_type, abstastic_flag
    ):
        ...
        # print("len of proposals_array", len(proposals_array[0]))

        # Filter proposals that satisfy the longitudinal velocity and acceleration
        ...
        # print("len of simulated_proposals_array", len(simulated_proposals_array[0]))
    ...
    def _get_closed_loop_trajectory(
        ...
        if self._iteration == 0:
            self.scenario_analysis = get_scenario_analysis(
                self._route_lane_dict,
                self._route_lane_dict,
                adjacent_lanes,
        ...
        scenario_type = scenario_turn(adjacent_lanes, *self.scenario_analysis)
        scenario_change = False
        if not scenario_type == self.last_scenario_type:
        ...
        simulated_proposals_array, original_indices, mask = (
            self._get_simulated_proposals(
                ego_state, chosen_lane, scenario_type, abstastic_flag
            )
        )
        ...
        if self._iteration%20==0:
            ...
            while max_score < threshold and try_times:
                chosen_lane, _, abstastic_flag = self._get_gpt_choice(
                    ego_state, adjacent_lanes, scenario_type, current_input, initialization
                )
                simulated_proposals_array, original_indices, mask = (
                    self._get_simulated_proposals(
                        ego_state, chosen_lane, scenario_type, abstastic_flag
            ...
            simulated_proposals_array, original_indices, mask = (
                self._get_simulated_proposals(ego_state, chosen_lane, scenario_type,abstastic_flag)
            )
        ...
        # tmp=np.copy(proposal_scores)
        # tmp[np.argmax(tmp)]=np.min(tmp)
