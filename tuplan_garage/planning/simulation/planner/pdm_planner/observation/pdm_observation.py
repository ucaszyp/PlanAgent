    ...
    def _get_traffic_light_geometries(
        ...
        for data in traffic_light_data:
            ...
            if (data.status == TrafficLightStatusType.RED) and (
                ...
                stoplines=lane_connector.stop_lines
                for i in range(len(stoplines)):
                    traffic_light_tokens.append(f"{self._red_light_token}_{lane_connector_id}_{i}")
                    traffic_light_polygons.append(stoplines[i].polygon)
