...
def sampled_tracked_objects_to_tensor_list(past_tracked_objects):
    ...
    """
    object_types = [TrackedObjectType.VEHICLE, TrackedObjectType.PEDESTRIAN, TrackedObjectType.BICYCLE, TrackedObjectType.TRAFFIC_CONE, TrackedObjectType.BARRIER]
    output = []
    output_types = []
    track_token_ids = {}
...
def agent_past_process(past_ego_states, past_time_stamps, past_tracked_objects, tracked_objects_types, num_agents):
    ...
    agents: <np.ndarray: num_agents, num_frames, 11>]].
        ...
        The num_frames includes both present and past frames.
    '''
    agents = np.zeros(shape=(num_agents, agents_tensor.shape[0], agents_tensor.shape[-1]+1), dtype=np.float32)
    ...
    for i, j in enumerate(indices):
        ...
        agents[i, :, agents_tensor.shape[-1]:] = int(agent_types[j])
...
def create_ego_raster(ax, vehicle_state):
    ...
    # Paint the rectangle
    rect = plt.Rectangle(ego_bottom_right, ego_front_length+ego_rear_length, ego_width, linewidth=2, color='#00CFBF', alpha=1.0, zorder=3,
                        transform=mpl.transforms.Affine2D().rotate_around(*(x_center, y_center), heading) + plt.gca().transData)
    plt.gca().add_patch(rect)
    plt.text((ego_bottom_right[0] + x_center) / 2, (ego_bottom_right[1] + y_center) / 2, '000', color='black', fontsize=8)

    plt.arrow(x_center, y_center, 
              (ego_front_length + ego_rear_length) * np.cos(heading), (ego_front_length + ego_rear_length) * np.sin(heading), 
              head_width=0.6, head_length=0.6, fc='r', ec='r', linewidth=2.5)

def create_agents_raster(ax, agents):
    for i in range(agents.shape[0]):
        if agents[i, 0] != 0:
            agent_type = agents[i, -1]
            ...
            rect = plt.Rectangle(agent_bottom_right, agent_length, agent_width, linewidth=2, color=color, alpha=1, zorder=3,
                                transform=mpl.transforms.Affine2D().rotate_around(*(x_center, y_center), heading) + plt.gca().transData)
            plt.gca().add_patch(rect)
            if agent_type == int(TrackedObjectType.VEHICLE):
                plt.text((agent_bottom_right[0] + x_center) / 2, (agent_bottom_right[1] + y_center) / 2, '%03d' %(i + 1), color='black', fontsize=8)
                plt.arrow(x_center, y_center, 
                        (agent_length - 1) * np.cos(heading), (agent_length - 1) * np.sin(heading), 
                        head_width=0.5, head_length=0.5, fc='r', ec='r', linewidth=2)
...
def create_map_raster(ax, lanes, crosswalks, route_lanes):
    ...
    for j in range(crosswalks.shape[0]):
        ...
        if crosswalk[0][0] != 0:
            ax.plot(crosswalk[:, 0], crosswalk[:, 1], 'b', linewidth=1) # plot crosswalk

    for k in range(route_lanes.shape[0]):
        route_lane = route_lanes[k]
        if route_lane[0][0] != 0:
            ax.plot(route_lane[:, 0], route_lane[:, 1], 'black', linewidth=1) # plot route_lanes
    print("draw maps")


def draw_trajectory(ax, ego_trajectory, agent_trajectories):
    # plot ego 
    ax.plot(ego_trajectory[:, 0], ego_trajectory[:, 1], 'r', linewidth=3, zorder=3)

    # plot others
    for i in range(agent_trajectories.shape[0]):
        if agent_trajectories[i, -1, 0] != 0:
            trajectory = agent_trajectories[i]
            ax.plot(trajectory[:, 0], trajectory[:, 1], 'm', linewidth=3, zorder=3)
